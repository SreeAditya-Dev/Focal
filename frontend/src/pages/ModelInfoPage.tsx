...
export const ModelInfoPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div>
        <h2 className="text-xl font-bold text-foreground">Focal Architecture & Benchmarks</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {metrics.map((m, idx) => (
          <div key={idx} className="glass-panel rounded-xl p-4 border border-border">
            ...
            <span className="text-xs text-muted-foreground mt-1 block">{m.benchmark}</span>
          </div>
        ))}
      </div>
      ...
      <div className="glass-panel rounded-2xl p-6 space-y-3">
        ...
        <ul className="list-disc list-inside text-xs text-popover-foreground space-y-2 leading-relaxed">
          <li>
            <strong className="text-foreground">Vector Temperature Scaling:</strong> Calibrates neural network logits to empirical frequencies, bounding Expected Calibration Error (ECE) below 4.2%.
          </li>
          <li>
            <strong className="text-foreground">Grad-CAM Heatmap Overlays:</strong> Hooks into the final convolutional layer activations to localize exactly where visual defects or blur regions occur.
          </li>
          <li>
            <strong className="text-foreground">Monte-Carlo Dropout:</strong> Executes 20 stochastic forward passes to surface uncertainty on ambiguous edge-case images.
          </li>
        </ul>
      </div>
    </div>
